          ...
          {loading ? (
            ...
            allTennisMatches.map((match) => (
              <tr key={match.id}>
                ...
                <td className='text-center'>{match.score_result}</td>
                <td>{(match.winner_name && match.winner_surname) ? `${match.winner_name} ${match.winner_surname}` : ""}</td>
                <td className={token && fullUser.role_id === 2 ? 'text-center' : 'btnsHidden'}><button className='goButtonDesign goButtonDelete' onClick={() => deleteThisTennisMatch(match.id)}><AiFillDelete /></button></td>
              </tr>
            ))
          ) : (
            <tr>
